...
## 17. Figs. A1/A2 relocated to fix reading order — the same problem the v2 self-audit already caught, recurred
...
## 18. Reviewer requests that had been answered only in summary; figure-label and facts.json fixes

Trigger: an external check of the resubmission against the original review listed items still incomplete. Each was verified against the files before acting. One claim was not upheld: the review said Fig. A2's "Figs. 1-2" label was outdated because "those maps are now Figures 3 and 4". In the current manuscript the two height maps are Figs. 1 and 2, so that label was correct; the review appears to have counted embedded images in an older build, where Figs. A1/A2 came first. The other items were upheld and fixed:

1. **Descriptive statistics (Reviewer #2, Major 4).** New script `09_supplementary_tables.py` writes `T_descriptive_statistics.csv` (mean, SD, min, p1-p99, max for ANBH, AGBH, built fraction and the four predictors, n=296,668), printed as manuscript Table A2. New findings reported in Section 5.1: the ANBH interquartile range is 2.4917-2.5094 m (1.8 cm wide), and 43,412 cells (14.6%) have ANBH = 0 (footprint present, no GHSL built signal). 43,111 of these are in the Low height / Low prior expansion class (45% of it), which explains that class's 1.35 m mean.
2. **Per-region statistics (Reviewer #4 point 1, Reviewer #6).** `T_regional_statistics.csv` gives per-region statistics for the 15 named areas, plus rows for all named regions combined, cells outside them, and the national sample. It is printed as Table A1. Membership uses the single `urban_region_name` assignment, so the 15 rows sum to the 28,129 named-region cells in the final sample (asserted in the script).
3. **Full cross-tabulations (Reviewer #4 point 4).** Script 06's tertile/quartile CSVs are re-expressed with marginals (`T_typology_crosstabs_with_marginals.csv`, Table A5). Disclosed tie rule: `np.digitize` puts a value equal to a cut-point in the upper class, unlike the median split. 22,560 cells tie at the lower growth tertile cut-point. The ANBH tertile cut-points are 2.4987 and 2.5036 m, 5 mm apart.
4. **Grid-resolution sensitivity (project design, Section 4 Level 2).** New script `10_grid_resolution_sensitivity.py` recomputes 250 m, 500 m and 1 km through one code path. It uses the project's national grids restricted to building_count > 0, the same 100 m UTM lattice and zone-rasterisation rule as script 02, GHSL BUILT-S 1975/2010/2015 nearest-warped onto that lattice, and CBD distance to the same 10 points as upstream script 07. 500 m reproduction check: ANBH/AGBH identical to script 02 (max diff 5e-7 m); built coverage r=0.997 against the upstream EPSG:4326 aggregation; Model C R² 0.2341 against 0.2346 published; n=296,677, because the recomputed coverage has none of the 9 missing values. Results (Table A4): R² 0.317/0.234/0.203; 2010 built coverage dominant at every resolution; growth 2010-2015 second at every resolution; ANBH IQR within 2.49-2.52 m everywhere. At 250 m, CBD distance (-0.056) ranks above growth 1975-2010 (0.045), and the Low/Low class median ANBH is 0.00 m, because 17.0% of 250 m cells are zero. Both differences are stated in Section 5.4.
5. **Moran's I inference (Reviewer #4 point 7).** Script 04 now uses 999 permutations with a fixed seed (was 99, unseeded). It writes `T_moran_diagnostics.csv` (I, E[I], permutation mean/SD, z_sim, p_sim, z_norm, p_norm), printed as Table A3, with a note under Table 2. All coefficients, R², λ and I values are identical to before (max abs diff 0.0); only p_sim changed (0.01 -> 0.001, the new floor). **Correction made:** the filtered-residual I for ANBH Model C (-0.013) has z = -10.2, so it is small but not statistically zero. Section 5.3's "adequately accounts for the spatial dependence" was replaced with "removes almost all of it", noting mild over-correction.
6. **Figures.** Fig. A1 had never been regenerated after the request for a dark version: the docx still embedded the v2 image, blue/orange with "(Sec. 2.1)" tags and a rotated four-line label. Its generation moved into script 08 (dark palette, horizontal labels, no section tags), and `build_docx.js` now points to `04_outputs/paper2_v3/figures/figA1_two_dimensions_schematic.png`. Fig. A2's final-sample box changed from "Tables 2-5" to "Tables 2-6, Figs. 3-4", and the side box no longer overlaps the main boxes. Embedded-image SHA-256 checked against the regenerated PNGs.
7. **Latent facts.json defect.** Seven curated entries (max-vs-p99, AGBH confound, predictor profile, Table 6 non-independence, 6-cell satellite check, figures, deposit expansion) had been edited into `facts.json` directly and were not produced by `build_facts_json.py`. Any re-run of `run_all.py` would have deleted them silently. They now live in `facts_curated.json`, which the builder deep-merges last. A diff of the rebuilt file against the previous one shows no lost keys; the only changed value is the Moran p_sim column. The stale "AI-generated guesses" wording in `provenance_agbh_anbh_2018.json` (written by an older run of script 01, whose current text says "automated guesses") was updated to match script 01.
8. **Still open, not claimed as done:** independent building-height or storey-count validation. The 114 remaining manual-validation cells are uncoded, and there is no LiDAR, field, or street-level comparison. Limitation 7 now says so in its first sentence.
